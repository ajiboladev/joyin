...
import { doc, getDoc, collection, orderBy, query, onSnapshot, getDocs, limit, startAfter, getCountFromServer, writeBatch,  setDoc, updateDoc, deleteDoc, increment, serverTimestamp, runTransaction } from "https://www.gstatic.com/firebasejs/12.6.0/firebase-firestore.js";
...
// Set up click handlers for post buttons (like, comment, share)
 async function setupPostInteractions(postElement, post, postId) {
    // ============================================
  ...
  try {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if (likeSnap.exists()) {
        ...
        transaction.update(userRef, { likesCount: increment(-1) });
         likeBtn.innerHTML = `<i class="fas fa-spinner fa-spin"></i> UnLiking...`;
      } else {
...
commentPostBtn.onclick = async () => {
      ...
      if (isPostingComment) {
    console.log("⏸️ Already posting comment...");
    return;
  }
...
const commentUser = commentUserData.data() ;
  ...
  try {
   ...
   // ALWAYS re-enable (whether success or error)
...
// const sheet = document.getElementById('commentSheet');
// const openBtn = document.getElementById('openCommentsTrigger'); // Your comment icon
// const closeBtn = document.getElementById('closeComments');
